...
<script>
  ...
  export default {
    ...
    computed: {
      ...mapState(['info', 'ratings']),    // 商家信息、评价
      ...mapGetters(['positiveSize']),     // 满意的评价 从 getters 中来 如下
        /* positiveSize (state) {
      ...
      filterRatings () {
        ...
        // 产生一个过滤新数组两个条件：选择全部、满意、不满意 跟 只看有内容的评价   这样的组合过滤，情况多种
        return ratings.filter(rating => {
          const {rateType, text} = rating
          /*
          条件1:
              selectType: 0/1/2   选择 满意、不满意、 全部
              rateType: 0/1     满意、不满意
              selectType===2 || selectType===rateType    选择了全部就返回全部不用看其他，选择了满意就返回满意的
          条件2
              onlyShowText: true/false
              text: 有值/没值
              !onlyShowText || text.length>0    onlyShowText 为 false 就用前面的，onlyShowText 为 true就用后面的
           */
          // 最后返回的是 两个条件都要满足的情况
